            ...
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    ...
                    let apii = await this.getFile(pp)
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '*НЕТ ОПИСАНИЯ*') :
...
this.sendButton(id, text, groupMetadata.subject, apii.data, [[(action == 'add' ? '💫 ДОБРО ПОЖАЛОВАТЬ 💫' : '☠ АУДИО ☠'), (action == 'add' ? '#welcomegc' : '#byegc')], ['♦ ГЛАВНОЕ МЕНЮ♦', `#menu`]], ftroli, {mentions: this.parseMention(text)})
...
*■ В этой группе нельзя не чего удалять..*
*■ Но вы можете наверное это отключить:*
*—◉ #выключить анти удаление*
*—◉ #включить анти удаления*
*—◉ Но вы можете еще скинуть мне я не кому не скажу
━━━━⬣  АНТИ УДАЛЕНИЕ ⬣━━━━
`.trim(), msg, {
