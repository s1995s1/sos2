...
if (/video/g.test(mime)) if ((q.msg || q).seconds > 8) return m.reply('*[❗ИНФО❗] СТИКЕР ИЗ ВИДЕО МОЖНО НЕ БОЛЕЕ 7 СЕКУНД*')
...
handler.help = ['stiker (caption|reply media)', 'stiker <url>', 'stikergif (caption|reply media)', 'stikergif <url>', 'стикер <url>']
handler.tags = ['sticker']
handler.command = /^(стикер|стикер1|стикер2|с|s)?(tic?ker)?(gif)?(wm)?$/i
